// ============================================================================
//  LicenseCrypto.h -- the small amount of cryptography Nebula Tide actually needs
//  for licensing. Adapted directly from SecondOut's LicenseCrypto.h (itself
//  from PerformLive's StoreCrypto.h, same company-wide signing key, same
//  verification approach) -- see that file for the full reasoning behind each
//  choice; not repeated here. Only the storage entropy string and the macOS
//  blob magic differ, per Amanorsac Studio/Security/license-integration-for-
//  developers.md section 6.
//
...
inline const char* storageEntropyString() { return "NebulaTide/license/v1"; }
...
inline juce::MemoryBlock protectSecret (const juce::MemoryBlock& plaintext)
{
    ...
    if (! CryptProtectData (&in, L"Nebula Tide", &entropy, nullptr, nullptr,
...
// "Nebula Tide wants to use your keychain", then Reaper, then every other host,
...
// Distinct from SecondOut's magic ('S','O','L','1') so a blob from a
// different Amanorsac product on the same account is rejected, not misread.
inline constexpr uint8_t kSecretMagic[4] = { 'A', 'L', 'P', '1' };
