// ============================================================================
//  LicenseKeys.h -- Nebula Tide's trust anchor for Amanorsac licensing.
//
//  This is the PUBLIC half of the shared Amanorsac signing key: an
//  uncompressed NIST P-256 point (0x04 || X || Y). The SAME key signs every
//  Amanorsac product's license proofs -- one trust anchor, company-wide (see
//  Amanorsac Studio/Security/license-integration-for-developers.md and
//  SecondOut's LicenseKeys.h, from which these bytes were copied verbatim).
//
//  These bytes are the key the LIVE server at amanorsac.studio signs with.
//  The corresponding PRIVATE key never leaves the server and must never be
