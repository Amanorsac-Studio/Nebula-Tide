// ============================================================================
//  LicenseClient.h -- Nebula Tide's client for the Amanorsac licensing service.
//  Copied from SecondOut's LicenseClient.h (the studio's reference
//  implementation, see Amanorsac Studio/Security/license-integration-for-
//  developers.md section 6) with only the constants in that section changed.
//  No accounts, no sessions, no downloads -- a plugin purchase should not
//  require creating an account. The license key itself is the whole credential.
//
...
public:
     ...
     * development only - the DEFAULT must stay the real server, because a
     * dev fallback here means every genuine activation goes to a machine
     * that isn't there (see the studio's licensing notes, section 7).
     */
     ...
     * cached proof instead of consuming a second seat.
     */
                 ...
                 [safe = makeSafe(), key, cb = std::move (done)] (LicenseResult r)
        {
            ...
            if (r.ok)
            {
                ...
                else
                {
                    ...
                    // Say what is actually wrong instead of reporting success
                    // and leaving someone locked out with no explanation.
                    r = LicenseResult::failure ("This license couldn't be verified by this version of "
                                                "Nebula Tide. Please install the latest version and try again.",
     ...
     * another machine.
     *
     * REAL-TIME SAFE: this is the gate render() calls on the audio thread
     * (Nebula Tide keeps the pads silent until licensed; see
     * PluginProcessor.cpp), so it must never touch the non-atomic fields
     * below directly - those are written from the message thread inside
     * activate()'s callback. licensedFlag is the single atomic word
     * published for this; recomputeLicensedFlag() is what keeps it current.
     */
...
private:
     ...
     * again (no crash, no dialog on the audio thread - correction just stops
     * being applied, same as any other "can't do it right now" state).
     */
    ...
    static juce::File appDataDir()
    {
        ...
        auto base = juce::File::getSpecialLocation (juce::File::userApplicationDataDirectory);
       #endif
        auto dir = base.getChildFile ("Amanorsac Studio").getChildFile ("Nebula Tide");
        dir.createDirectory();
        return dir;
    }
     ...
     * serial or MAC address -- it's personal data under GDPR, it breaks when
     * a drive is upgraded, and a determined sharer defeats either kind of id
     * anyway. This limits casual sharing, which is the honest goal.
     */
    void loadDeviceKey()
    {
        auto f = deviceFile();
