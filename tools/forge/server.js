...
const server = http.createServer((req, res) => {
  ...
  // ── static UI ──
  // chord reference for the pad slots (generated by tools/chords/generate.js)
  if (req.method === 'GET' && p === '/chord-data.js') {
    const f = path.join(__dirname, 'chord-data.js');
    if (fs.existsSync(f)) {
      res.writeHead(200, { 'Content-Type': 'application/javascript' });
      return res.end(fs.readFileSync(f));
    }
    res.writeHead(200, { 'Content-Type': 'application/javascript' });
    return res.end('window.NEBULA_CHORDS = null;');
  }
